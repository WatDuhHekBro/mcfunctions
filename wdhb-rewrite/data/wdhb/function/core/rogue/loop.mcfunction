...
execute if items entity @s weapon.offhand minecraft:iron_sword run function wdhb:core/rogue/offhand/0a-try
...
execute if entity @s[scores={statDropIronSword=1..}] run function wdhb:core/rogue/drop/try
...
execute if entity @s[scores={statSneak=1}] run function wdhb:core/rogue/sneak/try
...
# Ticking -> Impulse
# 0 -> 1 (triggers impulse), then 3+ (hold in place, two ticking functions cancel each other out), then 2 (is the final, only happens when you stop sneaking)
execute if entity @s[scores={statClick=1}] run function wdhb:core/rogue/click/try
...
advancement revoke @s only wdhb:click-iron-sword
